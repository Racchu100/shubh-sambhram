...
export default function HostLoginPage() {
  ...
  const handleLogin = () => {
    ...
    if (cleanPin === "SHUBH99") {
      const demoEvent = matched || DEFAULT_EVENTS[0];
      const requiredPass = demoEvent.hostPassword || "";
      if (requiredPass && cleanPass !== requiredPass) {
        setError("Invalid Host Password for the Demo Event.");
        return;
      }
      sessionStorage.setItem("host_auth_SHUBH99", "true");
      setError("");
      router.push(`/host/SHUBH99`);
      return;
    }

    if (matched) {
      const requiredPass = matched.hostPassword || "";
      if (requiredPass && cleanPass !== requiredPass) {
        setError("Invalid Host Password. Please try again.");
        return;
      }

      if (matched.status === "draft") {
        const updated = events.map((e) => (e.id === matched.id ? { ...e, status: "live" as const } : e));
        try {
          localStorage.setItem("sa_events", JSON.stringify(updated));
        } catch {}
      }
      
      sessionStorage.setItem(`host_auth_${cleanPin}`, "true");
      setError("");
      router.push(`/host/${cleanPin}`);
    } else {
      setError("Invalid Event PIN. Please contact the Super Admin.");
    }
  };
    ...
    <div className="luxury-light" style={{ minHeight: "100vh", display: "flex", flexDirection: "column", paddingTop: 0 }}>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem 1rem", marginTop: "70px" }}>
        <div className="glass-panel" style={{ width: "100%", maxWidth: "420px", padding: "2.5rem 2rem", textAlign: "center", position: "relative", overflow: "hidden" }}>
          ...
          <h2 className="gradient-text-gold" style={{ fontSize: "1.8rem", fontWeight: 800, marginBottom: "8px", fontFamily: "Playfair Display, Georgia, serif" }}>Event Host Portal</h2>
          <p style={{ color: "#555", fontSize: "0.88rem", marginBottom: "24px", lineHeight: 1.4 }}>
            Enter your Event PIN and Host Password to access and host your games.
          </p>

          <div className="input-group" style={{ textAlign: "left", marginBottom: "12px" }}>
            <label htmlFor="host-pin" style={{ fontSize: "0.78rem", fontWeight: 600, color: "#666", textTransform: "uppercase", letterSpacing: "1px" }}>Event Access PIN</label>
            <input
              type="text"
              id="host-pin"
          ...
          <div className="input-group" style={{ textAlign: "left", marginTop: "12px", marginBottom: "20px" }}>
            <label htmlFor="host-password" style={{ fontSize: "0.78rem", fontWeight: 600, color: "#666", textTransform: "uppercase", letterSpacing: "1px" }}>Host Password</label>
            <input
              type="password"
              id="host-password"
              className="input-field"
          ...
          <div style={{ marginTop: "24px", padding: "10px", background: "rgba(216, 178, 39, 0.05)", border: "1px dashed rgba(216, 178, 39, 0.25)", borderRadius: "12px" }}>
            <p style={{ color: "#666", fontSize: "0.8rem", margin: 0 }}>
              Tip: For the demo event, use PIN <code style={{ color: "#a6800b", fontWeight: 700, fontSize: "0.85rem" }}>SHUBH99</code> (leave password blank).
            </p>
          </div>
        </div>
      </div>
